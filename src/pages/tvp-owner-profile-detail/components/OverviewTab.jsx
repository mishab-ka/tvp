  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-card rounded-lg border border-border p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="text-center p-4 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-foreground">{owner?.vehicles?.length}</div>
              <div className="text-sm text-muted-foreground">Total Vehicles</div>
            </div>
            <div className="text-center p-4 bg-muted rounded-lg">
              <div className="text-2xl font-bold text-success">{owner?.vehicles?.filter(v => v?.status === 'active')?.length}</div>
            ...
            {owner?.vehicles?.slice(0, 3)?.map((vehicle) => (
              <div key={vehicle?.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  vehicle?.status === 'active' ? 'bg-success text-success-foreground' : 'bg-error text-error-foreground'
                }`}>
                  {vehicle?.status}
                </span>
              </div>
        ...
        {/* Financial Snapshot */}
        <div className="bg-card rounded-lg border border-border p-6">
          <h3 className="text-lg font-semibold text-card-foreground mb-4">Financial Snapshot</h3>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
              ...
              <span className="font-semibold text-success">${owner?.totalDeposits?.toLocaleString()}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
              ...
              <span className="font-semibold text-warning">${owner?.outstandingBalance?.toLocaleString()}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
              ...
              <span className="font-semibold text-primary">${owner?.monthlyEarnings?.toLocaleString()}</span>
            </div>
